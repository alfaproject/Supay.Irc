...
namespace Supay.Irc.Messages {
  ...
  /// <summary>
  /// Writes <see cref="Supay.Irc.Messages.IrcMessage"/> data to a <see cref="TextWriter"/> in irc protocol format.
  /// </summary>
  public class IrcMessageWriter : IDisposable {
    ...
    private String GetParamValue(int index) {
      ...
      if (thisParamAsCollection != null) {
        ...
        return Supay.Irc.Messages.MessageUtil.CreateList(thisParamAsCollection, seperator);
      }

      IList thisParamAsList = thisParam as IList;
      if (thisParamAsList != null) {
        String seperator = this.listParams[index.ToString(CultureInfo.InvariantCulture)];
        return Supay.Irc.Messages.MessageUtil.CreateList(thisParamAsList, seperator);
      }
